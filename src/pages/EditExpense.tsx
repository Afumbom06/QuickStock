...
export function EditExpense() {
  ...
  const { expenses, updateExpense, t, user, isOnline } = useApp();
  ...
  useEffect(() => {
    if (expense) {
      const isStandardCategory = ['transport', 'restock', 'bills', 'utilities', 'other'].includes(expense.category.toLowerCase());
      
      setFormData({
        category: isStandardCategory ? expense.category.toLowerCase() as ExpenseCategory : 'other',
      ...
      <div className="h-full flex items-center justify-center">
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg text-gray-900 mb-2">Cannot Edit Synced Expense</h3>
            <p className="text-gray-500 mb-4">
              This expense has been synced and cannot be edited. 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(isOnline ? 'Expense updated & synced!' : 'Expense updated (will sync when online)');
  ...
  return (
    <div className="h-full flex flex-col space-y-6 pb-6 overflow-y-auto">
      ...
      <div className="flex items-center gap-4 flex-shrink-0">
        <Button variant="ghost" size="sm" onClick={() => navigate(`/expenses/${expense.id}`)} className="gap-2">
          <ArrowLeft className="w-4 h-4" />
          Back
        </Button>
        <div>
          <h1 className="text-2xl text-gray-900">Edit Expense</h1>
